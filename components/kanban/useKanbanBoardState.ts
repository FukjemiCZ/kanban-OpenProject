...
function applySwimlineLocalPatch(
    card: WorkPackage,
    field: GroupField,
...
export function useKanbanBoardState(initialItems: WorkPackage[]) {
    ...
    const statusCandidatesByColumn = useMemo(() => {
        const map = new Map<BoardColumnKey, Array<{ name: string; href?: string; count: number }>>();
        const agg = new Map<BoardColumnKey, Map<string, { name: string; href?: string; count: number }>>();
        ...
        for (const item of items) {
            ...
            if (ex) ex.count += 1;
            else inner.set(k, { name: item.statusName, href: item.statusHref, count: 1 });
        }
        ...
        async (drag: DragCardPayload, target: BoardDropTarget) => {
            ...
            if (patchSpec.statusColumnKey) {
                ...
                if (candidate?.href) {
                    ...
                    // fallback necháme vyřešit serverem přes OPENPROJECT_COLUMN_STATUS_MAP
                    // klient pošle jen cílový sloupec
                }
            }
            ...
            try {
                const res = await fetch(`/api/work-packages/${card.id}`, {
                    ...
                    body: JSON.stringify({
                        ...
                        links: linkPatch,
                        boardMove: patchSpec.statusColumnKey
                            ? { statusColumnKey: patchSpec.statusColumnKey }
                            : undefined,
                    }),
                });
